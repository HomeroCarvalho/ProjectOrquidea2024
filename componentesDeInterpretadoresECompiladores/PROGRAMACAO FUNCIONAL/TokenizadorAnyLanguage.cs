...
namespace parser
{
    public class TokenizadorAnyLanguage
    {
        ...
        public List<string> GetTokens(string codigo, List<string> tokensTermosChave, List<string> tokensOperadores)
        {
            ...
            parser.Tokenizer.ComparerTexts comparerTexts = new Tokenizer.ComparerTexts();
            ...
            {
                Tokenizer.RetiraEmptyWords(ref ids);
                todosTokensEncontrados.AddRange(ids.ToList<string>());
            }

            textCopy = (string)codigo.Clone();

            List<parser.Tokenizer.TokenComPosicao> tokensNaoOrdenados = new List<Tokenizer.TokenComPosicao>();
            for (int umToken = 0; umToken < todosTokensEncontrados.Count; umToken++)
            {
                int index = textCopy.IndexOf(todosTokensEncontrados[umToken]);
                parser.Tokenizer.TokenComPosicao tokenComPosicao = new Tokenizer.TokenComPosicao(todosTokensEncontrados[umToken], index);
            ...
            parser.Tokenizer.ComparerTokensPosicao comparer = new Tokenizer.ComparerTokensPosicao();
