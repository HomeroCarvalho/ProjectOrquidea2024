...
{
    /// <summary>
    /// continuação da classe [Expressao].
    /// </summary>
    public partial class Expressao
    {
        ...
        {
            List<string> tokens = Tokenizer.GetTokens(this.ToString());
            return tokens;
        } // Convert()
        ...
        /// </summary>
        /// <param name="expressao">expressao a validar.</param>
        /// 
        /// <returns>[true] se a expressao é condicional.</returns>
        public static bool ValidaExpressaoCondicional(Expressao expressao)
        {
            if (expressao.tipoDaExpressao == "bool")
            {
                return true;
            }


            List<string> operadoresCondicionais = new List<string>() { ">", "<", ">=", "<=", "==", "!" };
            
            for (int i = 0;i<operadoresCondicionais.Count;i++)
            {
                if (expressao.tokens.Contains(operadoresCondicionais[i]))
                {
                    return true;
                }
            }
        ...
        public bool IsExpressaoAritimeticoUnario(Expressao exprss, Escopo escopo)
        {
            ...
            for (int x = 0; x < exprss.Elementos.Count; x++)
            {
                ...
                if (exprss.Elementos[x].GetType() == typeof(ExpressaoOperador))
                {
                    ...
                    if ((op.tipoRetorno == "int") && (op.tipo.Contains("UNARIO")))  
                    {
                        hasOperatorUnary++;
