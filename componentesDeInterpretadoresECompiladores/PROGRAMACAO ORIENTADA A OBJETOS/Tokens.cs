...
namespace parser
{
    public class Tokens
    {
        ...
        {
            return Tokenizer.GetTokens(Util.UtilString.UneLinhasLista(this.codigo));
        ...
        {
            char aspas = '\u0022';
            public Testes() : base("testes de tokenizacao")
            {

            }

            public void TesteTokens(AssercaoSuiteClasse assercao)
            {
                string code_0_0 = aspas + "Prompt.sWrite(" + aspas + "desenhando" + aspas + ")" + aspas;
                List<string> tokens = new Tokens(code_0_0).GetTokens();
