...
namespace parser
{
    ...
    /// <summary>
    /// classe wrapper ainda em processo de implementacao. não está contida como WrapperData neste versão do projeto.
    /// </summary>
    public class MultList
    {
        ...
        public void InsertAt(Objeto objToAdd, Vector indices)
        {
            ...
            for (int x = 0; x < indices.VectorObjects.Count; x++)
            {
        ...
        {
            AtomList lista = (AtomList)this;
            for (int x = 0; x < indices.VectorObjects.Count -1; x++)
            {
                lista = lista.list[(int)indices.Get(x)];
            }

            return lista.list[indices.VectorObjects.Count - 1].atom;
        }
